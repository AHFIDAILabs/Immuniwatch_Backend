...
export async function listOrganizations(
  ...
  try {
    ...
    const countMap = new Map(
      userCounts.map((u: { _id: unknown; count: number }) => [
        u._id?.toString(),
        u.count,
      ]),
    );
...
export async function getOrganization(
  ...
  try {
    ...
    const [users, postsToday, postsTotal, hitlPending, hitlTotal, alerts] =
      await Promise.all([
    ...
    res.json({
      ...
      claimLink:
        !org.adminClaimed && org.claimToken
          ? orgClaimLink(org.claimToken)
          : null,
    });
  } catch (err) {
    next(err);
...
export async function getPlatformOverview(
  ...
  try {
    ...
    const activityMap = new Map(
      (orgActivity as { _id: unknown; postsToday: number }[]).map((a) => [
        a._id?.toString(),
        a.postsToday,
      ]),
    );
    ...
    const userMap = new Map(
      (userCounts as { _id: unknown; count: number }[]).map((u) => [
        u._id?.toString(),
        u.count,
      ]),
    );
    ...
    const hitlMap = new Map(
      (hitlByOrg as { _id: unknown; pending: number }[]).map((h) => [
        h._id?.toString(),
        h.pending,
      ]),
    );
